...
VerticalScrollView.defaultProps = {
	...
	debug_useDefaultDynamicMargins = UIBloxConfig.verticalScrollViewUseDefaultDynamicMargins
}
...
VerticalScrollView.validateProps = t.strictInterface({
	...
	-- Debug Prop
	debug_useDefaultDynamicMargins = t.optional(t.boolean),
})
...
function VerticalScrollView:init()
	...
	self.getPadding = function(offset)
		...
		if self.props.paddingHorizontal then
			return UDim.new(0, self.props.paddingHorizontal - offset)
		else
			if self.props.debug_useDefaultDynamicMargins then
				return self.paddingBinding:map(function(absoluteWidth)
					return UDim.new(0, getPageMargin(absoluteWidth) - offset)
				end)
			else
				return UDim.new(0, PADDING_HORIZONTAL - offset)
			end
		end
	end
end
