return function()
	...
	local BadgeStates = require(UIBloxRoot.App.Indicator.Enum.BadgeStates)
	...
	it("should create and destroy with an empty badge value without errors", function()
		local frame = Instance.new("Frame")
		local originalAcceptString = UIBloxConfig.allowSystemBarToAcceptString
		UIBloxConfig.allowSystemBarToAcceptString = true
		local originalAddIsEmpty = UIBloxConfig.addIsEmptyToBadge
		UIBloxConfig.addIsEmptyToBadge = true
		...
		local element = mockStyleComponent({
			Roact.createElement(SystemBar, {
				itemList = {{
					...
					iconOff = Images["icons/actions/favoriteOff"],
					onActivated = function() end,
					badgeValue =  BadgeStates.isEmpty,
				}},
				selection = 1,
				placement = Placement.Left,
				hidden = false,
		...
		local badge = frame:FindFirstChild("Badge", true)
		local textLabel = badge:FindFirstChildWhichIsA("TextLabel", true)
		expect(textLabel.Text).to.equal("")

		Roact.unmount(instance)

		UIBloxConfig.allowSystemBarToAcceptString = originalAcceptString
		UIBloxConfig.addIsEmptyToBadge = originalAddIsEmpty
	end)
end
